...
// SupportedPlayModes 获取支持的播放模式
func (pm *playlistManager) SupportedPlayModes() []types.Mode {
	return slices.Collect(maps.Keys(pm.playModes))
}
...
func (pm *playlistManager) LoadState() error {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			slog.Error("panic when playlistManager.LoadState", slog.Any("err", r))
		}
	}()
...
func (pm *playlistManager) saveStateAsync() {
	...
	defer func() {
		if r := recover(); r != nil {
			// 在测试环境或storage未初始化时忽略错误
			slog.Error("panic when playlistManager.saveStateAsync", slog.Any("err", r))
		}
	}()
