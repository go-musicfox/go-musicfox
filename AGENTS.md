...
- **封面图**：Kitty 封面图在应用背景预留的透明区域渲染，位于文本和单元格背景之下；popup 仅覆盖与其重叠的部分。背景排除矩形仅覆盖图像实际填充的前 `rows-1` 行，Kitty 未填充的末行仍由主题 `AppBackground` 绘制。独立页面若需清理封面，必须在 deferred `MenuToPage.BeforeEnterMenuHook` 中调用 `coverRenderer.ClearDisplayed()`，使删除发生在 loading 的 Main 帧之后、目标页面切换之前；Last.fm API account 入口遵循此规则。tmux（≥3.3 且 `allow-passthrough on`）内通过 DCS `tmux;` 透传支持 Kitty 图形协议（`internal/ui/kitty/detector.go` 检测外层终端、生成器返回裸序列、写入点经 `kitty.Wrap` 的 `ansi.TmuxPassthrough` 包裹序列，并查询 `#{allow-passthrough}` 校验选项确已开启、未开启或查询失败时禁用透传）；pane 环境被 tmux 3.4+ 覆盖（`TERM_PROGRAM=tmux`）时回退 `tmux show-environment` 会话环境探测外层终端，GNU screen 仍不支持。**安全默认**：tmux 透传属实验性功能，即使终端能力检测通过，`CoverRenderer.IsEnabled` 仍要求 `main.lyric.cover.tmuxPassthrough=true` 才会在 tmux 内真正输出图像；默认关闭，以避免 Ghostty + tmux 大图透传导致的 GPU/WindowServer 卡死与 macOS watchdog 重启；开启后若检测到同一 session 有多个 tmux client 附着则自动禁用封面透传（避免 Kitty DCS 扇出到 Ghostty+Kitty 等引发 CPU/GPU 卡死）。开启后仍强制禁用旋转封面（仅静态封面），静态源图缩放到 320px，封面写入不再 `Stdout.Sync()`，完整图像 DCS 受 768 KiB 单包、持续 1 MiB/s、1 MiB burst 的运行时预算约束；写入超过 200ms 或失败时按 5 秒起、最高 30 秒的退避冷却熔断。写耗时仅作为 terminal/GPU pressure proxy（终端/GPU 压力代理），不是真实 GPU 指标。`--debug`/`main.debug` 会在封面大包写入时记录字节、耗时、进程 RSS 与限流状态（事件驱动，无周期性 heartbeat）；慢写（>200ms）会 Warn 并 `slogx.Flush`。开启 `tmuxPassthrough` 后静态封面走 Kitty Unicode placeholder（`a=t` 传输 + `a=p,U=1` 虚拟放置，歌词行经 `CoverRenderer.PlaceholderSegment` 嵌入 `U+10EEEE` 占位符）：图像驻留在 tmux 文字网格中，切换 tmux window / zoom 时随缓冲区清除，无需周期性 `tmux` 可见性轮询。限流或拥塞熔断时跳过绘制且状态不误标成功（复用指数退避避免逐帧重试和重复分配）。非 tmux 路径仍为绝对 CUP + `a=T` 叠图，行为不变。
