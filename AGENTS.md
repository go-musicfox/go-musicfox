...
- **人机验证**：账号密码登录触发 `-462`（需人机验证）时，解析响应中的 `data.verifyId/verifyType/verifyToken/params`，`internal/ui/verify_page.go` 借助 `utils/app/verify.go` 封装的 frontrisk 接口（`/api/frontrisk/verify/getqrcode`、`/qrcodestatus`）获取并渲染验证二维码；用户需使用与登录账号一致的网易云音乐 App 扫码，轮询状态 0/10/20/21（待扫码/已扫码/验证成功/已失效），验证成功后自动重试原登录请求。
