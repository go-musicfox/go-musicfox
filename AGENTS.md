...
- **网页登录**：仅 macOS。Login 页「网页登录」入口通过 `internal/macdriver/webkit`（purego + objc 桥接，无 CGO）弹出原生 WKWebView 窗口加载 `https://music.163.com/#/login`，使用 `nonPersistentDataStore`（内存不落盘）。登录检测：`WKHTTPCookieStore getAllCookiesWithCompletionHandler:` 每 ~1s 轮询，出现 `MUSIC_U` cookie 即成功（HttpOnly，`document.cookie` 读不到）；block 回调运行在 WebKit 私有队列，只做数据 marshal 经 channel 回传。控制器 `internal/ui/login_webview_darwin.go` + 主线程派发 `login_webview_helper_darwin.go`（`MusicfoxWebviewLoginHelper`，仿 `internal/desktop_lyrics/helper_darwin.go` 的 performSelectorOnMainThread 模式）；页面 `login_webview_page.go` 仿 QR 登录页用 `tea.Tick` 轮询消费事件。窗口显示期间将全局 `SetActivationPolicy(Prohibited)`（`internal/runtime/runtime_darwin.go:26`）临时切换为 `Accessory` 并激活，关闭时恢复；所有退出路径（成功/用户关窗/取消）都恢复。拿到 cookie 后复用 `utils/app.ParseCookieFromStr` → `neteaseutil.SetGlobalCookieJar` → `apputils.RefreshCookieJar()` 链路校验，失败则保持窗口继续轮询。非 macOS 平台入口提示不支持（`login_webview.go` 桩）。
