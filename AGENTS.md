...
- **封面图**：Kitty 封面图在应用背景预留的透明区域渲染，位于文本和单元格背景之下；popup 仅覆盖与其重叠的部分。背景排除矩形仅覆盖图像实际填充的前 `rows-1` 行，Kitty 未填充的末行仍由主题 `AppBackground` 绘制。独立页面若需清理封面，必须在 deferred `MenuToPage.BeforeEnterMenuHook` 中调用 `coverRenderer.ClearDisplayed()`，使删除发生在 loading 的 Main 帧之后、目标页面切换之前；Last.fm API account 入口遵循此规则。tmux（≥3.3 且 `allow-passthrough on`）内通过 DCS `tmux;` 透传支持 Kitty 图形协议（`internal/ui/kitty/detector.go` 检测外层终端、生成器返回裸序列、写入点经 `kitty.Wrap` 的 `ansi.TmuxPassthrough` 包裹序列，并查询 `#{allow-passthrough}` 校验选项确已开启、未开启或查询失败时禁用透传）；pane 环境被 tmux 3.4+ 覆盖（`TERM_PROGRAM=tmux`）时回退 `tmux show-environment` 会话环境探测外层终端，GNU screen 仍不支持。**安全默认**：tmux 透传属实验性功能，即使终端能力检测通过，`CoverRenderer.IsEnabled` 仍要求 `main.lyric.cover.tmuxPassthrough=true` 才会在 tmux 内真正输出图像；默认关闭，以避免 Ghostty + tmux 大图透传导致的 GPU/WindowServer 卡死与 macOS watchdog 重启。开启后仍强制禁用旋转封面（仅静态封面），静态源图缩放到 320px，封面写入不再 `Stdout.Sync()`，完整图像 DCS 受 768 KiB 单包、持续 1 MiB/s、1 MiB burst 的运行时预算约束；写入超过 200ms 或失败时按 5 秒起、最高 30 秒的退避冷却熔断。写耗时仅作为 terminal/GPU pressure proxy（终端/GPU 压力代理），不是真实 GPU 指标。`--debug`/`main.debug` 会在封面大包写入时记录字节、耗时、进程 RSS 与限流状态（事件驱动，无周期性 heartbeat）；慢写（>200ms）会 Warn 并 `slogx.Flush`。开启 `tmuxPassthrough` 后静态封面走 Kitty Unicode placeholder（`a=t` 传输 + `a=p,U=1` 虚拟放置，歌词行经 `CoverRenderer.PlaceholderSegment` 嵌入 `U+10EEEE` 占位符）：图像驻留在 tmux 文字网格中，切换 tmux window / zoom 时随缓冲区清除，无需周期性 `tmux` 可见性轮询。tmux 专用放置与占位符接口隔离在 `internal/ui/cover_renderer_tmux.go`，令牌桶与拥塞冷却隔离在 `internal/ui/cover_renderer_limiter.go`；限流器用独立 `sync.Once` 初始化，禁止获取封面状态锁，以免调试大包写入与清理路径锁重入。缩窗清理按当前窗格裁剪旧矩形，删除虚拟封面时用 `d=I` 同时释放图像数据。`main.lyric.cover.tmuxSyncOutput` 默认 `false`，遇到 tmux 3.7c 右侧 pane 封面分格、重复条带或切歌提示期间失真时由用户手动开启，仅 `show` 与 `tmuxPassthrough` 同时开启且检测到 tmux 透传时安装输出包装器；可设为 `false` 关闭同步兼容处理，不按 tmux 版本号自动关闭，因为下游可能回移修复。已修复 tmux 可继续接收标准同步刷新。用户说明见 `docs/tmux-cover.md`。tmux 下通过 `internal/ui/kitty/output.go` 的 `TmuxOutput` 包装 Bubble Tea 最终输出：含 Unicode 图片占位字符的完整写入使用成对的 `CSI ?2026h/l` 同步刷新序列，使 tmux 从完整字符网格重绘，兼容 tmux 3.7c 组合字符可见性判断漏加 pane 横向偏移的问题。同步控制必须放在通知/弹窗合成及渲染器行差分之后；内嵌在歌词行中的控制序列会被合成器去掉，导致切歌提示显示期间封面失真、提示消失后才恢复。输出包装器保留终端文件描述符，不含图片占位字符的写入直接透传。`MUSICFOX_TMUX_INTEGRATION=1 go test ./internal/ui/kitty -run TestUnicodePlaceholderTmuxOutput` 使用独立 tmux 服务和 Python 3 PTY 校验实际发往终端的行列坐标，覆盖左右 pane、通知合成前后、单行更新和交换位置；每帧校验后才允许发送下一帧。非 tmux 路径仍为绝对 CUP + `a=T` 叠图，行为不变。
...
macOS 的 AppKit 测试进程必须在 `_test.go` 的 `init()` 中调用 `runtime.LockOSThread()`，保证 `TestMain` 的 `NSApplication.Run` 留在启动线程。构造会读取应用路径的 track manager 前，测试需初始化 `configs.AppConfig` 并在结束时恢复；不能仅依赖 `WithDownloadDir` 跳过默认配置读取。
