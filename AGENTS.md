...
| `internal/frontend/` | UI-agnostic 前端注册表：`frontend.go`（`Frontend` 接口 `ID`/`Name`/`Run(ctx, LaunchOptions)`、`LaunchOptions`、包级 `Register`/`ByID`/`Registered`，零业务依赖）、`registration/` 聚合器（空导入 ui/headless 触发注册，子包防 `ui → frontend → ui` 环）；前端经 `commands.runPlayer` 的 `resolveFrontendID()` 选择 |
