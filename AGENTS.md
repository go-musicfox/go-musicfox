...
- **人机验证**：`-462` 表示网易云风控人机验证（非"请求过于频繁"），映射为 `utils/struct.NeedVerify`。账号密码登录触发时，`internal/ui/login_page.go` 解析响应 `data.verifyId/verifyType/verifyToken/params` 并按 `verifyType` 分流：40=扫码验证，进入 `internal/ui/verify_page.go`，借助 `utils/app/verify.go` 封装的 frontrisk 接口（`/api/frontrisk/verify/getqrcode`、`/qrcodestatus`）获取并渲染验证二维码；用户需使用与登录账号一致的网易云音乐 App 扫码，轮询状态 0/10/20/21（待扫码/已扫码/验证成功/已失效），验证成功后自动重试原登录请求一次（`LoginPage.riskVerifyOpened` 限制自动重试循环，手动退出验证页后复位），仍失败则提示改用 Cookie 登录；20=短信验证、50=设备/环境风险，TUI 内无法完成，分别提示改用扫码或 Cookie 登录。验证与登录入口均经 `utils/app.ApplyLoginStrategy()` 注入反风控参数：SDK（netease-music）`util.ApplyRequestStrategy` 在源头生成随机 NMTID（覆盖固定串，避免设备指纹识别），邮箱登录不再模拟 `os=ios/appver=8.7.01`，与手机号/二维码统一 `os=pc` 身份；`utils/app/verify.go` 两个接口调用前自保障注入，不依赖登录流程前置。
