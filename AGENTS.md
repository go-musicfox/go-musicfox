...
- **下载格式识别**：网易云接口声明的歌曲类型可能与实际内容不一致（如云盘歌曲报 `mp3` 实为 FLAC）。`internal/track/format_detect.go` 的 `sniffFileFormat` 按内容魔数嗅探真实格式：`fLaC`/`OggS`/`RIFF..WAVE`/`ID3` 魔数，MP3 无固定魔数则用 `0xFFE0` 帧同步 + version/layer/bitrate/采样率字段合法性启发式识别（AAC ADTS 的 layer 保留值 00 被拒绝，M4A 等不误判）。下载持久化（`persistStream`）在文件写入完成后按嗅探结果决定最终后缀（`correctedDownloadPath`），并清理声明后缀的旧文件；已下载文件在再次下载时经 `correctDownloadedFile` 同样修正后缀。`supportedFileExtensions`（下载目录查找）为 `mp3`/`flac`/`wav`/`ogg`。无法识别的内容维持声明后缀兜底。
