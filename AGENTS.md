...
| `internal/wasm/` | WASM 插件运行时（实验性）：契约（`contract.go` JSON Request/Response）、manifest（`manifest.go`）、wazero 封装（`plugin.go`：alloc/run/dealloc 调用协议、单 uint64 打包结果、超时 watchdog、实例互斥串行化）、目录扫描 + SHA-256 校验 + 生命周期（`manager.go`） |
