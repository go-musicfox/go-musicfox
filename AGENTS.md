...
- **专辑收藏状态**：`internal/ui/album_subscription.go` 在登录成功后分页同步收藏专辑 ID，并用互斥锁维护进程内缓存；歌曲操作菜单据此只展示当前可执行的“收藏专辑”或“取消收藏专辑”。缓存尚未就绪时使用“管理专辑收藏”，执行时通过专辑动态接口读取服务端状态后切换；所有收藏写操作也会先核对服务端状态并更新缓存。右键点击列表项仅展示该项操作，点击空白区域才展示当前播放操作；返回 `nil` 页面的延迟操作必须附带 rerender 命令，以确保 `DeferWithLoading` 被及时执行。
