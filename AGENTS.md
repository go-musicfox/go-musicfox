...
- **封面图**：Kitty 封面图在应用背景预留的透明区域渲染，位于文本和单元格背景之下；popup 仅覆盖与其重叠的部分。背景排除矩形仅覆盖图像实际填充的前 `rows-1` 行，Kitty 未填充的末行仍由主题 `AppBackground` 绘制。独立页面若需清理封面，必须在 deferred `MenuToPage.BeforeEnterMenuHook` 中调用 `coverRenderer.ClearDisplayed()`，使删除发生在 loading 的 Main 帧之后、目标页面切换之前；Last.fm API account 入口遵循此规则。tmux（≥3.3 且 `allow-passthrough on`）内通过 DCS `tmux;` 透传支持 Kitty 图形协议（`internal/ui/kitty/detector.go` 检测外层终端、生成器返回裸序列、写入点经 `kitty.Wrap` 的 `ansi.TmuxPassthrough` 包裹序列）；pane 环境被 tmux 3.4+ 覆盖（`TERM_PROGRAM=tmux`）时回退 `tmux show-environment` 会话环境探测外层终端，GNU screen 仍不支持。tmux 模式下图片以外层终端绝对坐标定位（`tmux display -p` 查询 pane 偏移、单 DCS 内含 `\e7`/`\e8` 保存恢复外层光标），避免失焦时画到其他 pane；偏移查询失败时跳过绘制且状态不误标成功（按指数退避重试，1 秒起连续失败翻倍、30 秒封顶，动画帧数据仅在放置查询成功后传输、失败零输出），偏移缓存随窗口尺寸变化失效。
