...
- **3.9.11 歌单/云盘集群提取为第七个真实插件（批量主菜单项 + 参数化主菜单项生产示范 + 跨插件按 key 协作）**：`internal/plugins/playlist`——「歌单/云盘」集群 4 个菜单整体移出（`menu_user_playlist`/`menu_user_collection`/`menu_high_quality_playlists`/`menu_cloud`），每菜单嵌入 `ui.BaseMenu`、构造器收 `ui.BaseMenu`。**key 逐一保持与内置注册相同**：`user_playlist`/`user_collect`/`high_quality_playlists`/`could`——ui 侧跳入集群的调用点零改动（`menu_search_result.go` 仍按 key 跳 `user_playlist`，`ui.UserPlaylistOpts` 留在 ui 供其使用）。`m.svc.*` → BaseMenu 导出转发（`m.User()`/`m.ToLoginPage(enterMenuCallback(main))`）。集群内跨菜单跳转改走 `ui.BuildMenu`（`user_playlist`/`high_quality_playlists` 的 SubMenu 跳 `playlist_detail`，后者留在 ui）；`user_collect` 的子菜单经 `ui.MustBuildNoArg` 按 `album_sub_list`/`artists_sub_list` key 构建（跨插件按 key 协作——这两个 key 由 album/artist 插件注册）。4 个入口菜单各自声明主菜单项，其中 `user_playlist` 是参数化 provider，经 `RegisterMainMenuItemWith("user_playlist", "我的歌单", func(base ui.BaseMenu) ui.Menu { return ui.MustBuild("user_playlist", base, ui.UserPlaylistOpts{UserID: ui.CurUser}) })` 构造（与内置入口行为一致；`ui.CurUser` 常量留在 ui，`menu_add_to_user_playlist.go` 仍使用）。ui 侧：删除 4 个文件；`registry_registrations.go` 删除 4 个注册；`expectedMenuKeys` 移除 4 个 key；`menu_main.go` 删除内置「我的歌单/我的收藏/精选歌单/云盘」入口（索引 0/1/3/4，帮助索引 5→1）——**主菜单位置变更：4 个入口从内置索引 0/1/3/4 变为插件项，排在全部内置项之后**；`registry_test.go` 重接 `user_collect`/`high_quality_playlists`/`user_playlist` 引用（`TestBuildMenuOptsTypeMismatch`/`TestMustBuildNoArg`/`TestMenuNavigationSmoke` → 内置 `search_type`/`playlist_detail`；`TestRegisterMainMenuItemWithBuilder` 的 builder 目标改 `playlist_detail`）+ `builtinMenuCount` 6→2；`foxful_integration_test.go` 移除 `album_sub_list`/`artists_sub_list` 行为等价 test-double 与包级 init()（`NewUserCollectionMenu` 已移出 ui，ui 测试二进制不再构建该 key）。测试移到 `internal/plugins/playlist/`（4 key 经 `ui.BuildMenu` 构建 + key 断言 + user_playlist 带 userID 构建断言 + 集群内跳转边 user_playlist/high_quality_playlists→playlist_detail + user_collect 跨插件子菜单 key 断言 + 4 主菜单项注册断言含 user_playlist 的 builder 以 `ui.CurUser` 构造；测试二进制空导入 album/artist 插件以注册跨插件子菜单 key）；聚合器 `plugins.go` 空导入 playlist + `plugins_test.go` 新增 `TestBlankImportRegistersPlaylistPlugin`（含参数化主菜单项 builder 断言）。— 验证: make lint 0 issues · make test 绿（无 FAIL）· make build 绿 · gofmt 干净
