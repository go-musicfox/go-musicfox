...
func main() {
	...
	// 创建应用实例
	app, err := NewCoreApp(*configPath, *logLevel)
	if err != nil {
	...
	fmt.Println("MusicFox Core shutdown complete")
}

func printUsage() {
	fmt.Printf(`MusicFox Core v%s - 音乐播放器核心功能版本

`, Version)
	fmt.Println("Usage:")
	fmt.Println("  musicfox-core [flags] [command]")
	fmt.Println()
	...
	fmt.Println("  musicfox-core play /path/to/song.mp3")
	fmt.Println("  musicfox-core --config /path/to/config.yaml interactive")
	fmt.Println("  musicfox-core playlist create \"My Playlist\"")
}