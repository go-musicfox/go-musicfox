...
tmux 静态源图缩放至 320px；图像 DCS 单包上限 768 KiB，持续预算 1 MiB/s，突发预算 1 MiB。失败或超过 200ms 的写入触发 5–30 秒冷却。写耗时是拥塞代理，不能测量 GPU，也不能保证终端/GPU 永不挂起。调试日志仅按事件记录，无周期性 RSS 查询；macOS 的 RSS 子进程查询有超时。
