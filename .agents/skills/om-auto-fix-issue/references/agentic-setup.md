...
- The chain skills this skill invokes (`om-verify-in-repo`, `om-root-cause`, `om-fix`, `om-open-pr`, `om-auto-review-pr`; feature route: `om-auto-write-spec`, `om-auto-implement-spec`) load the rest of the config themselves.
- A missing required chain skill stops the run and names the skill to install (`om-open-pr` alone has an inline fallback — see `references/pr-finalize.md`).
