---
name: ask-anhoder
description: 自动路由并调用合适的 open-mercato/skills 技能（om-*）。当用户描述一个任务但不确定该用哪个技能、或需要端到端工作流串联时使用。路由唯一明确时自动加载目标技能并按其工作流执行，不只给建议；路由模糊（多个候选都合理）时先列出候选让用户选择，确认后再执行。
disable-model-invocation: true
---

# Ask Anhoder

你不必记住全部 36 个技能，按场景问这里。路由唯一明确时**自动调用**目标技能并执行（见「自动调用协议」），而不是只输出一条 `Next:` 建议；路由模糊（多个候选都合理）时不自动调用，先列出候选让用户选择（见「路由规则 1」）。
...
0. **先判唯一性，模糊先问**：意图与已有信息能唯一确定目标技能时直接进入第 1 步；若多个候选技能都合理（如"处理这个 PR"可指 `om-pr-autopilot` / `om-auto-fix-pr` / `om-auto-review-pr`，"评审"可指 `om-auto-review-pr` / `om-code-review`），**停下来把候选路由呈现给用户**——每个候选附一句话适用场景并标注你的推荐，等用户选定后再调用，不自行拍板。
...
5. **停下等待用户的情况**：(a) 路由模糊，候选技能需用户选择（见路由规则 1）；(b) 目标技能自身要求人类介入——`om-brainstorm` 的路由确认、认领冲突询问、`--interactive` 模式、`om-ux-shape`/`om-ux-review-pr` 的对话环节；(c) 目标技能产出阻塞结论（`NO_ACTION_NEEDED`、`Status: blocked`、需用户决策的守卫）——向用户报告原因与证据，不自行绕行。

## 路由规则

1. **唯一路由才自动调用**：意图与信息能唯一确定目标技能时，给出路由的同时就自动调用（见自动调用协议），不罗列无关可能；**路由模糊（多个候选都合理）时不自动调用**，改为列出候选路由让用户选择——每个候选给一句话适用场景，标注你的推荐，用户选定后再执行。
