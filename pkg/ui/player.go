...
type Player struct {
    ...
    lrcTimer      *lyric2.LRCTimer // 歌词计时器
...
// PlaySong 播放歌曲
func (p *Player) PlaySong(song structs.Song, duration PlayDirection) error {
    loading := NewLoading(p.model)
    loading.start()
    defer loading.complete()

    table := db2.NewTable()
    _ = table.SetByKVModel(db2.PlayerSnapshot{}, db2.PlayerSnapshot{
...
func (p *Player) SetPlayMode(playMode PlayMode) {
    ...
    table := db2.NewTable()
    _ = table.SetByKVModel(db2.PlayMode{}, p.mode)
...
func (p *Player) updateLyric(songId int64) {
    ...
    lrcFile, _ := lyric2.ReadLRC(strings.NewReader("[00:00.00] 暂无歌词~"))
    defer func() {
        p.lrcTimer = lyric2.NewLRCTimer(lrcFile)
    ...
    if lrc, err := jsonparser.GetString(response, "lrc", "lyric"); err == nil && lrc != "" {
        if file, err := lyric2.ReadLRC(strings.NewReader(lrc)); err == nil {
            lrcFile = file
        }
    }
}
